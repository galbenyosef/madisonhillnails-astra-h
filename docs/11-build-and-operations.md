...
The current homepage uses `public/images/editorial-cherry.png` and `public/images/editorial-butter.png`, original AI-generated manicure inspiration. They are not photographs of the salon, its customers, or its work. The page labels them as creative imagery and includes a portfolio/shade disclaimer. The older `public/images/color-study.png` is retained as an unused first-design asset. Interactive polish samples are CSS artwork. Icons come from Lucide; typography uses local system fonts, so no third-party font request is needed. See [the editorial redesign record](13-editorial-redesign.md) for asset prompts, approved direction, motion behavior, and verification.
