...
`public/images/color-study.png` is the original AI-generated decorative polish still life prepared during this session. It is not a photograph of the salon, its customers, its products, or its work. The site labels it as color inspiration. The interactive nail silhouettes are CSS artwork. Icons come from Lucide; typography uses local system fonts, so no third-party font request is needed.
