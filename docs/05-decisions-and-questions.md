...
**Current authority:** [Approved build scope and cost constraints](10-approved-build-scope.md). The record below preserves earlier planning decisions; later owner instructions authorize implementation, remove cards/automatic fees, require authenticated booking, and select Better Auth with Supabase PostgreSQL on Free only. All services must remain $0; SMS verification is deferred. Earlier statements that implementation is unapproved or saved-card controls belong in MVP are superseded.
...
| D10 | Supabase/PostgreSQL custom scheduling + Stripe + durable transactional email | Concrete proposal for shared web/admin/app behavior and configurable saved-card fees; more implementation/maintenance responsibility | Recommendation awaiting owner approval; see architecture/backlog |
...
- On the owner's next instruction to continue, prepared the [architecture recommendation and prioritized backlog](09-architecture-and-mvp-backlog.md): one PostgreSQL scheduling authority, managed staff auth, Stripe saved-card/fee lifecycle, durable notification jobs, conflict prevention, and implementation dependencies. Recorded production database cost inputs and the increased responsibility of custom scheduling. This is a reviewable recommendation, not architecture acceptance or build approval.
- Owner subsequently authorized implementation and requested shared customer/admin authentication and signup-abuse protection. Explicitly selected removal of card collection from the first release, superseding disabled Stripe/card/fee controls. Initial local scaffolding began; no live service or deployment was configured.
- Owner preferred in-house application logic. Proposed Better Auth running in our application with PostgreSQL; Supabase and Turnstile are optional services, not prerequisites for a custom booking system.
- Owner accepted phone verification before the first online booking. Delivery requires an SMS provider and has separate usage costs; no provider account or paid texting was authorized.
- Owner accepted Supabase database hosting with Better Auth, conditional on it being free. Recorded Supabase Free as the selected database plan, its inactivity/backup limitations, and the separate unresolved SMS budget in [the current scope](10-approved-build-scope.md). No paid upgrade is authorized. Requested clarification on whether $0 also applies to texting.
- Owner clarified: “All services $0; defer SMS verification.” SMS verification is future work, not a first-release requirement. Retain email verification and account/booking abuse controls, subject to a validated free email allowance; authenticator-app admin two-factor authentication does not require SMS. Updated the current scope and working rules. No service was activated or purchased.
- Owner then instructed “lets build now.” Implemented the first website and custom booking/admin application, replacing the unfinished Supabase Auth scaffold with Better Auth and server-only PostgreSQL. Added local database/auth/mail tests and desktop/mobile browser checks. See [build/operations](11-build-and-operations.md) and [validation](12-validation.md). Live integration, real operating data, and production deployment remain pending; no paid service or actual customer message was sent.
- Owner rejected the first visual design and its subtle hero-only parallax, then approved a fashion editorial direction with dramatic manicure imagery, bold color on white, overlapping compositions, and pronounced layered parallax. Implemented [the editorial redesign](13-editorial-redesign.md) within the existing booking scope; the preview remains local and live booking setup remains pending.
- Owner confirmed that the salon also offers spa treatments, including pedicures. Updated the homepage introduction, service overview, local introduction, search/social metadata, and business-schema description to reflect nail care, pedicures, and spa treatments. Specific spa treatment names, pricing, durations, staff eligibility, and shared equipment capacity still need confirmation before configuring bookable services; no database service records or availability were invented.
- Owner rejected the parallax design and requested black “madison hill” lettering with “NAILS & SPA” beneath it at the same width. Updated the shared header/footer wordmark, removed the parallax controller and pause UI, and retained stationary editorial imagery. See the updated [design record](13-editorial-redesign.md). No booking logic or external services changed.
- Owner supplied `https://www.instagram.com/madisonhillnails/?hl=en` as the source of real salon nail images. Inspected the public profile and posts without login, selected four high-resolution nail-only photographs, and replaced the generated homepage/social imagery. Added original-post links, a profile link, and business-schema sameAs. Source URLs and dimensions are recorded in [photo provenance](14-salon-photography.md); no Instagram API integration or paid service was introduced.
- Owner requested a better hero with a clean white background and parallax, moving the retrieved salon photos into a gallery. This explicitly supersedes the earlier parallax-removal preference. Implemented a photo-free white hero with stationary headline/booking controls and moving decorative enamel shapes, plus a dedicated four-photo gallery and Gallery navigation. Black NAILS & SPA branding, source attribution, $0 services, and booking scope remain intact. See [the latest design record](15-white-hero-and-gallery.md).
