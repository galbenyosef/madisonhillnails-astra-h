...
- 8 Chromium browser tests passed across desktop and emulated iPhone viewports after the wordmark update and parallax removal. They check page errors, local structured data, preview noindex, responsive width, color selection, reduced motion, navigation, protected-route fallback, policies, and missing pages. The previous two parallax checks were retired with the removed motion controller.
